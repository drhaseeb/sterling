...
import { Settings as SettingsIcon, Save, UploadCloud, FileJson, TrendingUp, AlertCircle, Check, LogOut, Globe, Key } from 'lucide-react';
...
export default function Settings() {
  ...
  const handleSave = () => {
    ...
    if (firebaseConf.trim()) {
      ...
      if (!parsed || !parsed.apiKey || !parsed.projectId) {
        setStatus({ type: 'error', msg: 'Invalid Firebase Config. Please check format.' });
        return;
      }
      localStorage.setItem(STORAGE_KEY_CONFIG, JSON.stringify(parsed));
      setStatus({ type: 'success', msg: 'Saved! Reloading app...' });
      
      // Reload to trigger firebase.js re-initialization
      setTimeout(() => window.location.reload(), 1500);
    } else {
      setStatus({ type: 'error', msg: 'Firebase Config is required.' });
    }
  };
  ...
  const handleClear = () => {
    if(confirm("Are you sure? This will wipe all keys from this device.")) {
      localStorage.clear();
      alert("Keys cleared.");
      window.location.reload();
    }
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in">
      <div>
        ...
        <p className="text-slate-500 mt-1">
           {auth ? "Manage your preferences and keys." : "Welcome! Please paste your credentials to connect to your backend."}
        </p>
      </div>
      ...
      <div className="space-y-6">
         ...
         <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <label className="flex items-center gap-2 font-bold text-slate-900 mb-2"><UploadCloud size={20} className="text-teal-600"/> Firebase Config (Required)</label>
            <p className="text-xs text-slate-400 mb-3">Paste the full JavaScript object from Firebase Console.</p>
            <textarea 
               rows={6}
         ...
         <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                ...
                <input type="password" value={geminiKey} onChange={e => setGeminiKey(e.target.value)} placeholder="For Receipt AI" className="w-full bg-slate-50 p-3 rounded-xl border border-slate-200 outline-none text-sm" />
            </div>
            <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <label className="flex items-center gap-2 font-bold text-slate-700 mb-2"><TrendingUp size={18}/> Finnhub API Key</label>
                <input type="password" value={finnhubKey} onChange={e => setFinnhubKey(e.target.value)} placeholder="For Live Stocks" className="w-full bg-slate-50 p-3 rounded-xl border border-slate-200 outline-none text-sm" />
            </div>
         </div>
         ...
         <div className="pt-6 border-t border-slate-200 flex justify-between items-center">
            ...
            <button onClick={handleClear} className="text-rose-500 font-bold hover:text-rose-700 text-sm">
              Reset All Keys
            </button>
         </div>
      </div>
